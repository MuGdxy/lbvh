...
int example0()
{
    ...
    lbvh::bvh<float, float4, aabb_getter> bvh;
    bvh.objects() = ps;
    bvh.build();
    ...
    thrust::for_each(
        ...
        [bvh_dev = bvh.viewer()] __device__(std::size_t idx)
        {
            ...
            for(std::size_t i = 1; i < 10; ++i)
            {
                ...
                const auto num_found =
                    lbvh::query_device(bvh_dev, lbvh::overlaps(query_box), buffer, 10);
    ...
    thrust::for_each(thrust::device,
                     ...
                     [bvh_dev = bvh.viewer()] __device__(const unsigned int idx)
                     {
                         ...
                         const auto nest = lbvh::query_device(
                             bvh_dev, lbvh::nearest(self), distance_calculator());
